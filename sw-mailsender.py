...
# Should return a Header representing
# '=?utf-8?q?System_Wyborczy_Samorz=C4=85du_Studenckiego_PWr?=\n <informatyzacja@samorzad.pwr.edu.pl>'
def header_from():
...
def header_to(address):
    them = Header('Anonimowy głosujący', 'utf-8')
    them.append(f"<{address}>", 'ascii')
    return them
...
def main():
    polls = query_db_all("""select id, name, sending_out_to, sent_to, mail_template, closes_on_date
                      from polls 
