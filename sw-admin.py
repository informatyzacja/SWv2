...
def admin_sendout():
    poll_id = int(request.args.get('id'))
    row = query_db_one('select name, possible_recipients, sending_out_to, sent_to, mailing_active from polls where id=%s and owner_user=%s',
                        [poll_id, int(current_user.get_id())])
    if not row:
        return "Głosowanie nie istnieje", 400
...
def setup_static_poll_index_html(poll_id):
    ...
    with open(poll_path + "/index.html", 'w') as vote_index:
        vote_index.write(render_template('vote.html', name=name, options=options, choice_type=choice_type, closes_on_date=closes_on_date))
...
def admin_sendout_activatemailing_post():
    ...
    db_execute('update polls set mailing_active=true where id=%s and owner_user=%s', [poll_id, int(current_user.get_id())])
    get_db().commit()
