from flask import Flask, request, jsonify, g, render_template, redirect, flash
from flask_login import LoginManager, login_required, current_user, login_user, logout_user
import psycopg2
import psycopg2.extras  # use dicts and lists as json in postgres
import json
...
def setup_static_poll_index_html(poll_id):
    name, options, choice_type, closes_on_date = query_db_one(
        'select name, options, choice_type, closes_on_date from polls where id=%s and owner_user=%s',
...
def admin_editpoll_post():
    ...
    name, sending_out_to = query_db_one('select name, sending_out_to from polls where id=%s and owner_user=%s',
                                        [poll_id, int(current_user.get_id())])
...
def admin_sendout():
    poll_id = int(request.args.get('id'))
    row = query_db_one(
        'select name, possible_recipients, sending_out_to, sent_to, mailing_active from polls where id=%s and owner_user=%s',
        [poll_id, int(current_user.get_id())])
    if not row:
        return "Głosowanie nie istnieje", 400
    name, possible_recipients, sending_out_to, sent_to, mailing_active = row
...
def admin_sendout_queueselected_post():
    ...
    row = query_db_one('select sending_out_to from polls where id=%s and owner_user=%s',
                       [poll_id, int(current_user.get_id())])
    if not row:
        return "Głosowanie nie istnieje", 400
    ...
    db_execute('update polls set sending_out_to=%s where id=%s and owner_user=%s',
               [sending_out_to, poll_id, int(current_user.get_id())])
    get_db().commit()
...
def admin_sendout_activatemailing_post():
    ...
    db_execute('update polls set mailing_active=true where id=%s and owner_user=%s',
               [poll_id, int(current_user.get_id())])
    get_db().commit()
...
def admin_sendout_deactivatemailing_post():
    ...
    db_execute('update polls set mailing_active=false where id=%s and owner_user=%s',
               [poll_id, int(current_user.get_id())])
    get_db().commit()
...
def admin_results():
    ...
    voted_len = int(subprocess.Popen(['du', '--inodes', '-sS', f"/opt/sw/poll/{poll_id}/results/"],
                                     stdout=subprocess.PIPE).stdout.read().decode('utf8').split('\t')[0]) - 1
